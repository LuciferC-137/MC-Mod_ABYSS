...
public class Temper extends EntityModel<TemperRenderState> implements ArmedModel {
	...
	@Override
	public void setupAnim(TemperRenderState state) {
		root().getAllParts().forEach(ModelPart::resetPose);
		if (!state.onGround){
			this.flap(2f);
		}
		animate(state.attackAnimationState, TemperAnimation.attack, state.ageInTicks);
		HEAD.xRot = HEAD.xRot + state.xRot * ((float)Math.PI / 180F);
		HEAD.yRot = HEAD.yRot + state.yRot * ((float)Math.PI / 180F);
