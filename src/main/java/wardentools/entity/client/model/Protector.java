...
public class Protector extends EntityModel<ProtectorRenderState> {
	...
	@Override
	public void setupAnim(ProtectorRenderState state) {
		this.resetPose();
		root().getAllParts().forEach(ModelPart::resetPose);
		if (!(state.isSpawning) || state.isDispawning) {
			if (!state.isSprinting) {
				animateWalk(ProtectorAnimation.walking, state.walkAnimationPos,
						state.walkAnimationSpeed, 1f, 2.5f);
			} else {
				animateWalk(ProtectorAnimation.running, state.walkAnimationPos,
						state.walkAnimationSpeed, 1f, 2.5f);
			}
			animate(state.earTickle, ProtectorAnimation.earsTickle, state.ageInTicks);
			animate(state.attackAnimationState, ProtectorAnimation.hit, state.ageInTicks);
		...
		animate(state.spawning, ProtectorAnimation.spawn, state.ageInTicks);
		animate(state.dispawning, ProtectorAnimation.dispawn, state.ageInTicks);
	}
}