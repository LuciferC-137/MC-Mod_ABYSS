...
public class Noctilure  extends EntityModel<NoctilureRenderState> {
	...
	@Override
	public void setupAnim(@NotNull NoctilureRenderState state) {
        float speedFactor = Math.min(Mth.abs(state.speed) * 37f, 1.5f); // the factor 37f is empirical
        animate(state.standing, NoctilureAnimation.standing, state.ageInTicks);
        animate(state.walking, NoctilureAnimation.walking, state.ageInTicks * speedFactor);
        animate(state.flying, NoctilureAnimation.fly, state.ageInTicks
                * (state.isFlightSprinting ? 3f : 1f) );
        animate(state.landing, NoctilureAnimation.landing, state.ageInTicks);
        animate(state.idleFlying, NoctilureAnimation.idleFly, state.ageInTicks);
        animate(state.idleFlyToFly, NoctilureAnimation.idleFlyToFly, state.ageInTicks);
        this.animateWingsOnFall(state);
        this.animateNeckToLookAt(state.xRot, state.yRot);

	}

    private void animateWingsOnFall(NoctilureRenderState state) {
