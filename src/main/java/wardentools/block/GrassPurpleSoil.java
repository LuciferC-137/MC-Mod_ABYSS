...
public class GrassPurpleSoil extends Block {

    public GrassPurpleSoil(Properties properties) {
        super(properties);
    }
    ...
    public void randomTick(@NotNull BlockState blockState, @NotNull ServerLevel level,
        ...
        if (level.getBlockState(blockPos.above()).isCollisionShapeFullBlock(level, blockPos.above())) {
            level.setBlockAndUpdate(blockPos, BlockRegistry.PURPLE_SOIL.get().defaultBlockState());
        }
    }
